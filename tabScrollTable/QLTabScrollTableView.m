...
@interface QLTabScrollTableView ()
{
...
@implementation QLTabScrollTableView
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshTableReloadData:) name:QLNeedRefreshTableReloadDataNotification object:nil];
...
- (void)refreshTableReloadData:(NSNotification*)notification
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            {
                if([notification.object isEqual:self->subScrollView1])
                {
                    freshTable = self->tableView1;
                }
                else if([notification.object isEqual:self->subScrollView2])
                {
                    freshTable = self->tableView2;
                }
                else if([notification.object isEqual:self->subScrollView3])
                {
                    freshTable = self->tableView3;
                }
                else if([notification.object isEqual:self->subScrollView4])
                {
                    freshTable = self->tableView4;
                }
            }
...
- (void)setTabTitleArray:(NSArray *)tabTitleArray
{
    ...
    QLBigClickButton *view = [[QLBigClickButton alloc]initWithFrame:CGRectMake(0.f, _headView.frame.origin.y+_headView.frame.size.height, self.frame.size.width, 55.f)];
    view.backgroundColor = [UIColor whiteColor];
    CGFloat titleLbWid = self.frame.size.width/self.tabTitleArray.count;
    
    for(int i = 0;i<self.tabTitleArray.count;i++)
    {
        QLBigClickButton *sectionTitleLb = [[QLBigClickButton alloc] initWithFrame:CGRectMake(i*titleLbWid, 0.f,titleLbWid,55.f)];
        [sectionTitleLb setTitle:[self.tabTitleArray objectAtIndex:i] forState:UIControlStateNormal];
        [sectionTitleLb setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
        sectionTitleLb.titleLabel.font = [UIFont systemFontOfSize:12.f];
    ...
    QLBigClickButton *btOne = (QLBigClickButton *)[view viewWithTag:TabBarBtnTag];
    UIImageView *selectLine = [[UIImageView alloc] initWithFrame:CGRectMake(btOne.frame.origin.x+(btOne.frame.size.width-10.f)/2.f,55.f-3.5f, 10.f, 1.5f)];
    selectLine.backgroundColor = [UIColor darkGrayColor];
    selectLine.layer.cornerRadius = 1.f;
...
- (void)tabSelectAction:(id)sender
{
    ...
    [self tabSelectIndex:self.currentIndex animated:YES];
...
- (void)tabSelectIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    [tabScrollView setContentOffset:CGPointMake((tabScrollView.contentSize.width/self.tabCount)*self.currentIndex, tabScrollView.contentOffset.y) animated:animated];
...
- (void)generateTableView:(UIView *)view index:(NSInteger)index
{
    ...
    newtTabel.estimatedRowHeight = 0;
    newtTabel.estimatedSectionHeaderHeight = 0;
    newtTabel.estimatedSectionFooterHeight = 0;
...
- (void)setTabViewArray:(NSArray *)tabViewArray
{
    ...
    {
        UIView *view = (UIView *)[tabViewArray objectAtIndex:i];
